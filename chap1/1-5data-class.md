...
> **注意：データだけのクラス自体が悪いわけではない。** データの受け渡し専用のクラス（DTO / Record）は正当な設計パターンであり、Java の `record` や Python の `@dataclass` など言語レベルでもサポートされている。  
> **問題になるのは、** 本来ビジネスロジック（計算・検証ルール）を持つべきクラスなのに、フィールドを並べるだけで「ただの入れ物」になってしまい、ロジックがクラスの外にバラバラに散らばっている状態のこと。
...
> データ（フィールド）と、そのデータを操作するロジック（メソッド）を **一つのクラスにまとめて閉じ込め** 、外部から直接データを触れないようにする考え方。`private` でフィールドを隠し、メソッドだけを公開することで、データが勝手に壊されるのを防ぐ。
...
> 「税込金額 ＝ 税抜 ＋ 税」のように、プログラムの実行中 **常に成り立っていなければならないルール** のこと。カプセル化によってこのルールをクラス内部で保証することで、外部のコードがルールを破ることを防ぐ。
...
# ⚠️ Python には Java のような厳密な private は存在しない
# _（アンダースコア1つ）は「外部から触らないでください」という開発者間の"お約束（慣習）"
# 技術的にはアクセスできてしまうが、慣習を破ると意図しないバグの原因になる
# contract._amount_excluding_tax = 99999  # 慣習的にやってはいけない
```
